...
const RunningProducts: React.FC = () => {
  ...
  // Lọc sản phẩm chạy bộ
  const runningProducts = products.filter((product) =>
    product.categories.some(
      (category) =>
        category.name.toLowerCase().includes("chạy") ||
        category.name.toLowerCase().includes("giày")
    )
  );

  // Sử dụng runningProducts nếu có, nếu không thì sử dụng tất cả sản phẩm
  const displayProducts =
    runningProducts.length > 0 ? runningProducts : products;
