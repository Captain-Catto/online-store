...
const CasualProducts: React.FC = () => {
  ...
  // Lọc sản phẩm mặc hằng ngày
  const casualProducts = products.filter((product) =>
    product.categories.some((category) => category.name === "Áo")
  );

  // Sử dụng casualProducts nếu có, nếu không thì sử dụng tất cả sản phẩm
  const displayProducts = casualProducts.length > 0 ? casualProducts : products;
