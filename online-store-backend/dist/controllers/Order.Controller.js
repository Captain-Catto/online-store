...
exports.getUserOrdersByAdmin =
  exports.calculateShippingFeeForCart =
  exports.cancelOrder =
...
const ProductInventory_1 = __importDefault(
...
const createOrder = async (req, res) => {
  ...
  try {
    ...
    if (voucherId) {
      const voucher = await Voucher_1.default.findByPk(voucherId, {
        transaction: t,
      });
      if (!voucher) {
        await t.rollback();
    ...
    for (const item of orderItems) {
      ...
      // Cập nhật tồn kho
      const inventory = await ProductInventory_1.default.findByPk(
        item.inventoryId,
        {
          transaction: t,
        }
    ...
    for (const item of orderItems) {
      ...
      const productDetail = await ProductDetail_1.default.findByPk(
...
const cancelOrder = async (req, res) => {
  ...
  try {
    ...
    for (const detail of orderDetails) {
      ...
      console.log(
        `Đã thêm sản phẩm ID ${detail.productId} vào danh sách cần cập nhật`
      );
      const productDetail = await ProductDetail_1.default.findOne({
        where: {
      ...
      if (productDetail) {
        ...
        if (inventory) {
          ...
          console.log(
            `Cập nhật tồn kho: ${inventory.getDataValue("stock")} + ${
              detail.quantity
            } = ${newStock}`
          );
    ...
    console.log(
      `Có ${updatedProductIds.size} sản phẩm cần kiểm tra trạng thái`
    );
    // Xử lý cập nhật trạng thái sản phẩm
    // Xử lý cập nhật trạng thái sản phẩm
    if (updatedProductIds.size > 0) {
      const updatePromises = Array.from(updatedProductIds).map(
        async (productId) => {
          ...
          if (product) {
            console.log(
              `Sản phẩm ID ${productId}, trạng thái hiện tại: ${product.status}`
            );
            ...
            if (totalStock > 0 && product.status === "outofstock") {
              console.log(
                `Cập nhật sản phẩm ID ${productId} từ ${product.status} sang active`
              );
              ...
              try {
                ...
                const updatedProduct = await Product_1.default.findByPk(
                  productId,
                  {
                    transaction: t,
                  }
                ...
                console.log(
                  `Sản phẩm ID ${productId} sau khi cập nhật có trạng thái: ${updatedProduct?.status}`
                );
                ...
                console.error(
                  `Lỗi khi cập nhật sản phẩm ID ${productId}:`,
                  updateError
                );
                return {
                  productId,
